...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($name) || empty($phone)) {
    ...
    $error["profile"] = "Invalid File Format"; 
...
?>
   ...
   * Gallery photo 
   */
  const gallInput = document.getElementById("gallery-photo");
  const gallPrevPhoto = document.querySelector(".gall-preview");

  gallInput.onchange = (e) => {
    $fileDataGall = e.target.files;

    let gallPrevItem = "";

    for (let $i = 0; $i < $fileDataGall.length; $i++) {
      const galfileUrl = URL.createObjectURL($fileDataGall[$i]);
      ...
      gallPrevPhoto.innerHTML = gallPrevItem;
    }
